...
flags.DEFINE_integer('n_iterations', 15000, 'number of iterations')
...
#           FLAGS.batch_size * FLAGS.print_every / (time.time() - t0)))
      print('Iteration: {0:d} ELBO: {1:.3f}'.format(
          i,
          np_elbo / FLAGS.batch_size))
      t0 = time.time()
  ...
  if FLAGS.latent_dim == 2:
    os.system(
        'convert -delay 100 -loop 0 {0}/posterior_predictive_map_frame*png {0}/posterior_predictive.gif'
        .format(FLAGS.logdir))
    os.system(
        'convert -delay 100 -loop 0 {0}/prior_predictive_map_frame*png {0}/prior_predictive.gif'
        .format(FLAGS.logdir))
